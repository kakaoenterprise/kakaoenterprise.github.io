...
오늘날 많은 기업에서는 챗봇을 이용해 고객 응대 서비스를 제공하고 있습니다. 콜센터와 비교해 구축 비용은 더 저렴하면서도 여러 명의 고객을 실시간으로 동시 응대할 수 있기 때문입니다. 이 덕분에 고객은 대기자가 많아 상담사 연결까지 한참을 기다리거나 수많은 상품 소개 메뉴를 찾아다닐 수고를 덜게 됐습니다. 이런 챗봇은 대개 매뉴얼에 따라 질문을 입력해야 사용자가 원하는 답을 하도록 구현돼 있습니다. 자신이 가진 문제나 질문을 해결하려는 용도로 챗봇을 이용하는 사용자에게는 감정적 교감보다는 사용자가 원하는 답을 내어주는 일이 더 중요해서죠.

하지만 이런 챗봇이 발전을 거듭해 과중된 업무에 치이는 상담사를 도와줄 응대 능력을 갖추려면, 궁극적으로는 인간의 감정에 공감할 수 있어야 합니다. 감정은 인간이 누군가와 대화를 하고 사회적인 관계를 맺는데 주된 역할을 합니다. 따라서 챗봇이 마치 사람 같은 반응을 보인다면 사용자는 매우 큰 만족감을 느끼게 되겠죠. 앵무새같이 말하는 무심한 챗봇보단, 내가 처한 상황에 공감해주면서 해답을 제시해주는 챗봇에 누구라도 더 큰 호의를 가질 수밖에 없으리라는 판단은 바로 이런 이유 때문입니다.
...
<em>[ 표 1 ] 텍스트 스타일 변환 예시 문장. (슬픔), (기쁨), (화남), (크리스마스)처럼 텍스트로 표현하는 이모지도 전환할 수 있다.</em>
...
카카오엔터프라이즈는 입력 문장에서 '스타일' 토큰을 명시적으로 삭제하고, '내용 토큰과 타깃 스타일 속성값만을 가지고 문장 벡터를 생성하는 접근 방식에서 영감을 얻었습니다. 카카오엔터프라이즈가 고안한 SST 모델 또한 1)문장에서 ‘스타일' 토큰을 삭제한 뒤, 2)’콘텐츠’ 토큰만을 가지고 입력 문장과는 반대되는 스타일 속성값을 가진 문장을 생성합니다. 다만 기존과는 다른 방식으로 각 모듈을 구현했는데, 이는 각 단락서 이어서 하겠습니다.

{% include image.html name="003.png" width="" align="center" %}
<em>[ 그림 2 ] 문장의 스타일 속성값을 긍정에서 부정, 또는 부정에서 긍정으로 전환해 문장을 생성하는 STT 모델의 동작 과정</em>
...
{% include image.html name="005.png" width="" align="center" %}
<em class="center">[ 그림 4 ] STT 모델에서 두 손실 함수를 이용한 훈련 과정</em>
