...
오늘날 많은 기업에서는 챗봇을 이용한 고객 응대 서비스를 제공하고 있습니다. 콜센터를 구축하는 비용보다 훨씬 더 저렴하고, 더 많은 고객을 동시에 응대할 수 있는 효용성을 갖춘 덕분입니다. 그 결과, 고객은 대기자가 많아 상담사 연결까지 한참을 기다리거나 수많은 상품 소개 메뉴를 찾아다닐 수고를 조금은 덜 수 있게 됐습니다.

보통 이런 챗봇은 매뉴얼에 따라 질문을 입력해야만 사용자가 원하는 답을 하도록 구현돼 있습니다. 자신이 가진 문제나 질문을 해결하려는 용도로 챗봇을 이용하는 사용자에게 '챗복과의 교감적 대화'는 별로 중요한 기능이 아니기 때문입니다.

하지만 챗봇이 과중한 업무에 치이는 상담사의 완벽한 업무 파트너로 발전하기 위해서는 공감적 대화 능력이 앞으로 더 많이 요구될 거로 보입니다. 감정은 인간이 누군가와 대화를 하고 사회적인 관계를 맺는데 주된 역할을 합니다. 따라서 챗봇이 마치 사람 같은 반응을 보인다면 사용자는 매우 큰 만족감을 느끼게 되겠죠. 앵무새같이 말하는 무심한 챗봇보다는, 내가 처한 상황에 적절한 감정적 반응'도' 보이는 챗봇에 대화할 맛이 나겠죠.
